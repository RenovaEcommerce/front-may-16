...
export const BlogFilter: React.FC = () => (
	<section className="relative max-w-[1440px] mx-auto w-full px-[10px] z-10  mb-10">
		<div className="flex justify-between flex-col md:flex-row inside-mb">
			<ul className="w-full h-max max-w-[1000px] overflow-x-auto no-scrollbar slider-gap flex justify-between">
				{blogFilter.map((item) => (
					<li key={item.id}>
						<Link
							href={item.link}
