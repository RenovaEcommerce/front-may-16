...
export default function App() {
    ...
    const [url,] = useState<string>('https://shawfloors.com/flooring/hardwood');
    const [category,] = useState<string>('hardwood');
    ...
    const handleExtractData = async (item: string | string[]) => {
        try {
            await axios.post('http://localhost:4100/products/add/hardwood', Array.isArray(item) ? item : [item])
                .then(response => {
