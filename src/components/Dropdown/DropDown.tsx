                        ...
                        dropdowns[idType].map((category, index) => (
                            <li
                                className={`text-title hover:text-main-yellow cursor-pointer ${
                                    index === selected ? "underline text-main-yellow" : ""
                                }`}
                                key={index}
                                onMouseEnter={() => {
                                    setSelected(index);
                            ...
                            >
                                <Link href={category.link}>
                                    <div className="flex gap-4">
                                        {category.text}
                                        {index === selected && (
                                            <Image src={arrow_yellow_right} alt="arrow" />
                                        )}
                                    </div>
                                </Link>
                            </li>
                        ...
                        <li className=" text-title text-primary" key={type.link}>
                            <Link
                                ...
                                className={`hover:text-main-yellow ${
                                    expandedTypes.includes(type.link) ? "underline text-main-yellow font-bold" : ""
                    ...
                    <div className="flex flex-wrap gap-5 items-start h-max  ">
                        {isId &&
                            ...
                            catalogOptionsHero[isId].map((link, index) => (
                                <Link className="catalog-button whitespace-nowrap ease-in-out " href={link.url} key={index}>
                                    {link.text}
                                </Link>
                            ))}
                    </div>
